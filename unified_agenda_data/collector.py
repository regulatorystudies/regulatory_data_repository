"""
Unified Agenda data collector — download and convert Reginfo.gov XML to CSV.
Logic extracted from unified_agenda_data_collector_updated.ipynb.
"""
...
def download_file(
    year: int, season: str, directory: str, status_callback=None
) -> Optional[str]:
    """Download XML for given year/season. Returns file path or None on error."""
    if year == 2012:
...
def get_latest_year_season():
    """Fetch latest Unified Agenda year and season from Reginfo.gov."""
...
def collect_ua_data(
    ...
    """
    Download XML and convert to CSV for the given year/season range.
    Saves CSV to directory and returns the combined DataFrame.
    """
    result_xml = []
    result_csv = []
