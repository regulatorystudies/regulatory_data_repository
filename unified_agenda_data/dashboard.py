...
with tab3:
    ...
    year = st.number_input("Enter Year here", min_value=1995, max_value=current_year-1, step=1, value=current_year-1)
    ...
    if year == current_year-1:
        ...
        season = ["spring","fall"]
    season = st.selectbox(
        "Please enter the season",season
    )
    ...
    if st.button("Run Analysis", key="tab3_run"):
        with st.spinner("Loading agenda data and building plots…"):
            try:
                summary, figures = ua_analysis.run_analysis_for_dashboard(year, season, current_year=current_year)
